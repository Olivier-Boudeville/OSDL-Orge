...
% Custom-made test <b>TCP/IP Orge client</b>.
%
...
run() ->
	...
	SecondClient =
		orge_tcp_client:start_link( SecondLogin, SecondPassword, TargetHost ),
	...
	SameAsFirstClient =
		orge_tcp_client:start_link( FirstLogin, FirstPassword, TargetHost ),

	?test_info( "Checking whether the first client failed "
				"in connecting twice." ),
	...
	%       ?test_info_fmt( "Current server state is: ~ts.", [StateString] )
