...
It is a higher-order energy, called the Mana, that can be consumed and converted to other kinds of energy. It can be to some extent controlled and channeled by some creatures, from the Arcanes to the world that creatures experience.

This energy seems to resist formalization and analysis, and still cannot be considered as being mastered at all by the scientific effort. However some generic empirical rules have been discovered and taught from a generation to the next.

Notably there are constants that can be found in most forms of magic: whether you utter a spell, perform arcane gestures, play and sing music, or write runes, you are manipulating a series of symbols that seem to have a shared meaning, beyond the mode of operation, i.e. the actual way they are brought to existence.
...
This language is still to be decyphered, and before ever studying its syntax and grammar, knowing whether it is based on an alphabet would be a tremendous progress indeed.
...
Many paths lead to arcane operations, but three of them are the main ones:
...
Besides these paths, other ones (called *Minor Paths* by the practioners of the three paths already mentioned) exist, like alchemy, augur, omen, presage and divination, shamanism, hex, jinxes, bewitching, thaumaturgics, trance, voodoo, vampirism, sortilege.
...
But there are also path-specific skills. The topmost ones are:
 ...
 - ``Magery``, from which all the skills of the ``Mage`` Path flows
 - ``Priesthood``, from which all the skills of the ``Cleric`` Path flows
 - ``Bardic Verse``, from which all the skills of the ``Bard`` Path flows


Each spell is generally tight to at least one Arcane Path: for example, when wanting to cast a Fireball spell, which is a spell typically in the ``Mage`` path, one should have strong ``Magery`` skills to have a reasonable chance of casting it successfully.
...
As these three arcane paths are quite different, a wizard can hardly be good at more than one of them: this would involve having numerous dissimilar high characteristics and advanced skills, which usually cannot happen.
...
Therefore most wizards, once having selected the arcane path(s) they want to follow, not only invest on skills directly relative to that path (ex: they might increase their ``Magery`` skills if having selected the Mage Path), but also on ``Arcane Technics`` skills, i.e. on skills dedicated to a spell casting method that corresponds to the path and spells they want to favor. 

For example, if wanting to be able to cast high-end fireballs (obviously in the Mage path), then a wizard should increase its skills in spell casting methods that can apply to fireballs, like Incantations or Gesture SpellCasting, rather than Bardic Verse.
...
If the caster is not time-constrained, meditations and rituals can ease his incantations. In some case, rituals are strictly necessary.
...
Gesture Spellcasting is the natural way of using arcane powers for Gnomes. Other species are seldom comfortable with this approach, except some Humans, who sometimes make uses of staffs to improve the upstroke of their gesture.
...
Because poetry and ballads do not deal blows, its is unfairly despised by many. 

However a well-interpreted bardic verse, either only sung, or accompanied with 
an instrument, can have tremendous yet different effects on allies and opponents as well: for example, whereas the former will have their willpower reinforced and their mental fatigue alleviated, opponents will be prone to desillusion, fear and desertion.

Moreover, all beings in the area (i.e. opponents, allies, and the caster himself) may suffer from Sonic damages. 

This form of arcane operation is still considered weak, but if its effects have less direct impact during battle, they can affect numerous opponents while letting the caster making use of weapons (with penalties, though).

Indeed most spells cast through bardic verse will have for implicit target the members of the caster's party that are in hearing range, as the recitation will be aimed directly to them, talking only to their spirit and to their heart.

Between fights, verses can soothe physical and psychological wounds, help allies resting or opponents being put on sleep.

The Bard Path is of course the main path to rely on bardic verses, although clerics can from time to time sing ballads to celebrate their deities. 
